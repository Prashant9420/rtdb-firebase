...
export default function SignInSide() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Save username and current date to the database
    await updateUserData(userinfo.uid,userinfo.email.split("@")[0]);
      localStorage.setItem('userData',JSON.stringify(userinfo));
  ...
  const updateUserData = async (uid,email) => {
    try {
      ...
      console.log('User data addded successfully');
    } catch (error) {
